...
MiniToolbox 提供了完整的插件生态系统，支持插件的开发、打包、分发和安装。本文档将指导你如何开发、发布和分享 MiniToolbox 插件。
...
```javascript
const themeResult = await api.invoke('mt.secure-call', {
  channel: 'ui.getTheme'
});

if (themeResult.ok) {
  const { theme, effective } = themeResult.data;
  console.log('当前主题:', theme, effective);
}
```
